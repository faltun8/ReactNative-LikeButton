...
import {MyBanner, NewsCard} from './components';
...
const News = () => {
    ...
    return (
        <SafeAreaView>
            ...
            <FlatList
            keyExtractor= {(item, index) => item.id.toString()} // important for performance
            data = {news_data}
            renderItem = {renderNewsData}
            numColumns={2}
            ListHeaderComponent={<MyBanner data={banner_data}/>}
            />
        </SafeAreaView>
    )
}
